...
gh run list --status=failure --limit 1 --repo NachoColl/mateocoll-website

# 2. View failure logs
gh run view <run-id> --log --repo NachoColl/mateocoll-website | grep -i "error"
...
gh run view <run-id> --log-failed --repo NachoColl/mateocoll-website
```
